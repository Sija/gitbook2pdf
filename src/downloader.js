...
export class Downloader {
  ...
  async #expandMenuLinks(page) {
    await page.evaluate(() => {
      ...
      const elements = document.querySelectorAll(
        'a[data-rnwrdesktop-fnigne="true"] > div[tabindex="0"]',
      );
  ...
  async #preparePage(page) {
    await page.evaluate(() => {
      ...
      const sectionsToExpand = document.querySelectorAll(
        'div[aria-controls^="expandable-body-"]',
      );
      ...
      const lastModifiedEl = document.querySelector(
        'div[dir="auto"] > span[aria-label]',
      );
